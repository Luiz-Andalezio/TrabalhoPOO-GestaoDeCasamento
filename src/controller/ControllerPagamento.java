...
public class ControllerPagamento {
    ...
    public void controllerCrudPagamentos(GUI gui, Usuario usuarioLogado, EventoDAO eventodao, IgrejaDAO igrejadao, CartorioDAO cartoriodao, PessoaDAO pessoadao, FornecedorDAO fornecedordao, Pagamento pagamento, PagamentoDAO pagamentodao, ParcelaDAO parceladao, LocalDateTime calendario) {
                ...
                switch (menuPagamentoOpc) {
                        ...
                        if (opc == 1) {
                            ...
                            if (opcParcela == 1) {
                                ...
                                while (selecionaPagante < 1 || selecionaPagante > 2 || selecionaPagante == 0) {
                                    selecionaPagante = Integer.parseInt(JOptionPane.showInputDialog("Informe quem você deseja ser o pagante da " + parcela + "° parcela\n\n" + "1- Noivo: " + eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo().getNome() + "\n2- Noiva: " + eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva().getNome()));
                                    if (selecionaPagante == 1) {
                                        JOptionPane.showMessageDialog(null, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo().getNome() + ", o noivo, foi selecionado como o pagante desta " + parcela + "° parcela.");
                                        pagamento = pagamentodao.registraPagamento(null, descricao, calendario);
                                        parceladao.registraParcela(calendario, data, pagamento, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo(), valor, parcela);
                                    }
                                    if (selecionaPagante == 2) {
                                        JOptionPane.showMessageDialog(null, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva().getNome() + ", a noiva, foi selecionado como o pagante desta " + parcela + "° parcela.");
                                        pagamento = pagamentodao.registraPagamento(null, descricao, calendario);
                                        parceladao.registraParcela(calendario, data, pagamento, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva(), valor, parcela);
                                    }
                                }
                            }
                            //Pagamento parcelado
                                ...
                                while (parcela <= fornecedor.getParcelas()) {
                                    ...
                                    while (selecionaPagante < 1 || selecionaPagante > 2 || selecionaPagante == 0) {
                                        ...
                                        if (selecionaPagante == 1) {
                                            JOptionPane.showMessageDialog(null, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo().getNome() + ", o noivo, foi selecionado como o pagante desta " + parcela + "° parcela.");
                                            pagamento = pagamentodao.registraPagamento(fornecedor, descricao, calendario);
                                            parceladao.registraParcela(calendario, data, pagamento, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo(), fornecedor.getValorAPagar(), parcela);
                                        }
                                        if (selecionaPagante == 2) {
                                            JOptionPane.showMessageDialog(null, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva().getNome() + ", a noiva, foi selecionado como o pagante desta " + parcela + "° parcela.");
                                            pagamento = pagamentodao.registraPagamento(fornecedor, descricao, calendario);
                                            parceladao.registraParcela(calendario, data, pagamento, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva(), fornecedor.getValorAPagar(), parcela);
                                        }
                                    }
                                    parcela++;
                                }
                        ...
                        if (!"".equals(s)) {
                            ...
                            if (id != 0) {
                                ...
                                if (veredito == JOptionPane.YES_OPTION) {
                                    ...
                                    if (pagamentodao.retornaPagamentoByID(id, fornecedordao).getFornecedor() != null) {
                                        ...
                                        if (veredito == JOptionPane.YES_OPTION) {
                                            m = new StringBuilder("\nInforme o ID do fornecedor que deseja para mudar para o pagamento: \n\n")
                                                    ...
                                                    .append(pagamentodao.retornaPagamentoByID(id, fornecedordao).getFornecedor().getNome())
                                                    .append("\n");
                                            int idF = Integer.parseInt(JOptionPane.showInputDialog(m));
                                            if (idF == 0) {
                                                JOptionPane.showMessageDialog(null, "Fornecedor do pagamento abaixo mantido:\n\n " + pagamentodao.retornaPagamentoByID(id, fornecedordao));
                                            } else {
                                                Fornecedor fornecedorAtt = fornecedordao.recebeFornecedorByID(idF);
                                                pagamentodao.atualizaFornecedorByPagamento(id, fornecedorAtt, fornecedordao, calendario);
                                    ...
                                    if (!"".equals(descricaoAtt)) {
                                        pagamentodao.atualizaDescricao(id, descricaoAtt, fornecedordao, calendario);
                                        m = new StringBuilder("\nPagamento parcialmente atualizado: \n").append(pagamentodao.retornaPagamentoByID(id, fornecedordao));
                                        JOptionPane.showMessageDialog(null, m);
                                    } else {
                                        JOptionPane.showMessageDialog(null, "Nenhuma descricao enviadas...");
                                    }

                                    if (pagamentodao.contarParcelasPorPagamento(id, fornecedordao) != 1) {
                                        veredito = JOptionPane.showConfirmDialog(null, "Deseja editar as parcelas do pagamento abaixo?\n\n" + pagamentodao.retornaPagamentoByID(id, fornecedordao), "Confirmar Edição", JOptionPane.YES_NO_OPTION);
                                        if (veredito == JOptionPane.YES_OPTION) {
                                            int continua = 0;
                                            while (continua != -1) {
                                                int nmrParcela = Integer.parseInt(JOptionPane.showInputDialog("\nInforme o número da parcela que deseja editar do pagamento abaixo: \n\n" + parceladao.buscarParcelasPorPagamento(id, pagamentodao, fornecedordao, pessoadao) + "\n\n0 - Voltar"));
                                                if (nmrParcela == 0) {
                                                    continua = -1;
                                                } else {
                                                    String dataAtt = JOptionPane.showInputDialog("Digite a data que deseja atualizar da parcela abaixo: \n\n" + parceladao.buscarParcelaEspecifica(id, nmrParcela, pagamentodao, fornecedordao, pessoadao));
                                                    int i = 0;
                                                    while (i < 1 || i > 2 || i == 0) {
                                                        i = Integer.parseInt(JOptionPane.showInputDialog("Informe quem você deseja ser o pagante da parcela abaixo: \n\n" + parceladao.buscarParcelaEspecifica(id, nmrParcela, pagamentodao, fornecedordao, pessoadao) + "\n1- Noivo: " + eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo().getNome() + "\n2- Noiva: " + eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva().getNome()));
                                                        if (i == 1) {
                                                            JOptionPane.showMessageDialog(null, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo().getNome() + ", o noivo, foi selecionado como o pagante da parcela abaixo:\n\n" + parceladao.buscarParcelaEspecifica(id, nmrParcela, pagamentodao, fornecedordao, pessoadao));
                                                            parceladao.atualizaParcela(id, nmrParcela, dataAtt, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoivo(), pagamentodao, fornecedordao, pessoadao, calendario);
                                                        }
                                                        if (i == 2) {
                                                            JOptionPane.showMessageDialog(null, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva().getNome() + ", a noiva, foi selecionado como o pagante da parcela abaixo:\n\n" + parceladao.buscarParcelaEspecifica(id, nmrParcela, pagamentodao, fornecedordao, pessoadao));
                                                            parceladao.atualizaParcela(id, nmrParcela, dataAtt, eventodao.retornaEvento(igrejadao, cartoriodao, pessoadao).getPessoaNoiva(), pagamentodao, fornecedordao, pessoadao, calendario);
                                                        }
                                                    }
                                                }
                                            }
                    ...
                    case 4:
                        ...
                        if (!"".equals(s)) {
                            ...
                            if (id != 0) {
                                int veredito = JOptionPane.showConfirmDialog(null, "Deseja mesmo excluir este pagamento abaixo?\n\n" + pagamentodao.retornaPagamentoByID(id, fornecedordao), "Confirmar Exclusão", JOptionPane.YES_NO_OPTION);
                                if (veredito == JOptionPane.YES_OPTION) {
                                    JOptionPane.showMessageDialog(null, "Pagamento excluido com sucesso!\n\n" + pagamentodao.retornaPagamentoByID(id, fornecedordao));
                                    pagamentodao.excluirPagamento(id, parceladao, pagamentodao, fornecedordao, pessoadao);
                                } else {
